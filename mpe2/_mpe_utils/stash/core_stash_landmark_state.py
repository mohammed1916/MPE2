...
class LandmarkState(EntityState):
    """State for landmarks where position is write-once.

    The first assignment to `p_pos` (typically during initialization) is
    allowed; any subsequent attempt to change `p_pos` will raise an
    AttributeError to prevent moving landmarks after placement.
    """
    def __init__(self):
        super().__init__()
        # use an internal storage so we can control setting behavior
        self._p_pos = None
        # self.count_pos = 3
        self.c = None
    ...
    def p_pos(self, value):
        # allow first-time assignment (when _p_pos is None), but disallow
        # changing the position afterwards
        if getattr(self, "_p_pos", None) is None:
            self._p_pos = value
        else:
            # self.count_pos +=1
            # if self.count_pos == 3:
            traceback.print_stack()
            print("Changing p_pos: ", value)
            # raise AttributeError("Cannot change p_pos of Landmark after it has been set")
        print("End of Setter p_pos: ", value)
